...
@Component({
  ...
  template: `
    @if (authService.isAuthenticated()) {
      ...
      <div class="min-h-screen bg-standby-bg flex items-center justify-center">
        <div class="text-center space-y-4">
          <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-standby-accent-blue to-standby-accent-teal flex items-center justify-center mx-auto animate-pulse">
            <span class="text-white font-bold text-xl">S</span>
          </div>
          <p class="text-standby-text-muted">Authenticating...</p>
        </div>
      </div>
    }
  `,
...
export class AppComponent implements OnInit {
  // Inject the SSO Auth Service (configured in app.config.ts)
  ...
  ngOnInit(): void {
    ...
    // Setup listener to detect logout from LoginTerminal
